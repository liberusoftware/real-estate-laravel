...
| `NUXT` | **out of scope** | **out of scope** | a Nuxt 3 frontend is a separate consumer of this host's REST API, developed and deployed independently — it never appears in `composer.lock` and so is out of this audit's scope rather than ruled out. See `CLAUDE.md`'s Nuxt 3 Frontend section |
